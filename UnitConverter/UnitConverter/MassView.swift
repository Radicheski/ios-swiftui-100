...
struct MassView: View {
    ...
    @State private var unitOne: UnitMass = .baseUnit()
    @State private var valueTwo = 0.0
    @State private var unitTwo: UnitMass = .baseUnit()
    
    let options: [UnitMass] = [.kilograms, .grams, .decigrams, .centigrams, .milligrams,
                               .micrograms, .nanograms, .picograms, .ounces, .pounds, .stones,
                               .metricTons, .shortTons, .carats, .ouncesTroy, .slugs]
