...
struct AngleView: View {
    ...
    @State private var unitOne: UnitAngle = .baseUnit()
    @State private var valueTwo = 0.0
    @State private var unitTwo: UnitAngle = .baseUnit()
    
    let options: [UnitAngle] = [.degrees, .arcMinutes, .arcSeconds,
                                .radians, .gradians, .revolutions]
